...
public sealed class PlayerDataWriterGenerator : IIncrementalGenerator {
    ...
    private static readonly string SceneIdFullName = "SilkSongScenes.SCENES";
    private static readonly string NoIds = "EmptyCollections.EMPTY_STRING_ID_LOOKUP";

    private static Dictionary<string, string> fieldToStringIdDict = new() {
        ["scenesEncounteredBench"] = SceneIdFullName,
        ["scenesEncounteredCocoon"] = SceneIdFullName,
        ["scenesMapped"] = SceneIdFullName,
        ["scenesVisited"] = SceneIdFullName,
    ...
    private static void Generate(SourceProductionContext context, Compilation compilation, ImmutableArray<string> maps) {
        ...
        foreach (ObservedField field in supportedFields) {
            ...
            var hasIdDict = fieldToStringIdDict.TryGetValue(field.FieldName, out string? stringIdDict);

            void WriteIdDict() {
                if (!hasIdDict) {
                    // error
                    source.Append("MissingDictFor_" + field.FieldName);
                } else {
